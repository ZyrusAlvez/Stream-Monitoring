    ...
    try {
      if (isYouTubeChannelUrl(url)) {
        ...
        if (data) {
          ...
          const res = await runScraper(url, data.folder_id, "youtube/channel", config.repetition, config.interval, config.startTime);
          if (res) {
            toast.success("Youtube Channel URL submitted successfully and scraper started!");
          }
        }
      } else {
  ...
  return (
    <div className="flex flex-col items-center h-screen gap-4">
      ...
      <div className="flex w-[80%] gap-2">
        <InputText
          placeholder="Enter the youtube channel link here"
          value={url}
      ...
      <FolderReader type="youtube/channel" refreshKey={refreshKey} setRefreshKey={setRefreshKey}/>
    </div>
  );
};
